import { ActionAdventureAnime, fetchCrunchyRoll } from '@/api';
import React from 'react';
import { Text, View } from 'react-native';
import { MultiAnimeCardList } from '../MultiAnimeCardList';
import { AppSpinner } from '../Spinner';

export function CrunchyRoll() {
    ...
    React.useEffect(() => {
        const loadActionAdventure = async () => {
            try {
                ...
                setIsLoading(true);
                const data = await fetchCrunchyRoll();
    ...
    if (!animes || animes?.length === 0) {
        return (
            <View className='px-4 flex-row gap-3'>
                <Text>No Crunchy Roll series found.</Text>
            </View>
        );
    }
